...
class UrlManager extends BaseUrlManager
{
     ...
     */
    public $enableLanguagePersistence = false;
    ...
    public  function getCurLangCode()
    {
        $locale = Yii::$app->language;
        foreach ($this->languages as $code => $loc) {
            if ($loc == $locale) {
                if (is_numeric($code)) {
                    return $loc;
                }
        ...
        return false;
        /*$curCode = explode('-', $locale)[0];
        return $curCode;*/
    }

    public  function getLangUrl($url, $code = null)
    {
        if (empty($code)) {
            $code = $this->getCurLangCode();
        }
        $url = explode('/', $url);
        array_shift($url);
        if (!empty($url[0]) && isset(Yii::$app->params['availableLanguages'][$url[0]])) {
            array_shift($url);
        }
