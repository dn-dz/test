...
$photoTemplate = '<tr data-id="{{photo_id}}">'.(IS_ROOT ? '<td>{{photo_id}}</td>' : '').'\
    ...
    <td>';
 if (!empty(Yii::$app->params['availableLanguages'])) {
         $n = 0;
         foreach (Yii::$app->params['availableLanguages'] as $code => $name) {
             $n++;
             if($code == Yii::$app->params['defaultLanguage']) {
                $photoTemplate .= $code.'<textarea class="form-control photo-description">{{photo_description}}</textarea>';
             } else {
                 $photoTemplate .= $code.'<textarea data-lang="'.$code.'"  class="form-control photo-description">{{photo_description_'.$code.'}}</textarea>';
             }
          }
     }
    $photoTemplate .= '<a href="' . Url::to(['/admin/photos/description/{{photo_id}}']) . '" class="btn btn-sm btn-primary disabled save-photo-description">'. Yii::t('easyii', 'Save') .'</a>\
    </td>\
...
<table id="photo-table" class="table table-hover" style="display: <?= count($photos) ? 'table' : 'none' ?>;">
    ...
    <?php
    $needArray = ['{{photo_id}}', '{{photo_thumb}}', '{{photo_image}}'];

        foreach (Yii::$app->params['availableLanguages'] as $code => $name) {
            $n++;
            if($code == Yii::$app->params['defaultLanguage']) {
                $needArray[] = '{{photo_description}}';
            } else {
                $needArray[] = '{{photo_description_'.$code.'}}';
            }
        }
    ...
    <?php foreach($photos as $photo) : ?>
        <?php
            $replace = [$photo->primaryKey, Image::thumb($photo->image, Photo::PHOTO_THUMB_WIDTH, Photo::PHOTO_THUMB_HEIGHT), $photo->image];
        foreach (Yii::$app->params['availableLanguages'] as $code => $name) {
            $n++;
            if($code == Yii::$app->params['defaultLanguage']) {
                $replace[] = $photo->description;
            } else {
                $replace[] = $photo->{'description_'.$code};
            }
        }
