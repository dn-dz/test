...
class News extends \yii\easyii\components\API
{
    ...
    public function api_items($options = [])
    {
        if(!$this->_items){
            ...
            $query = NewsModel::find()->localized(Yii::$app->urlManager->getCurLangCode())->with($with)->status(NewsModel::STATUS_ON);
    ...
    public function api_last($limit = 1)
    {
        ...
        $result = [];
        foreach(NewsModel::find()->localized(Yii::$app->urlManager->getCurLangCode())->with($with)->status(NewsModel::STATUS_ON)->sortDate()->limit($limit)->all() as $item){
    ...
    {
        $news = NewsModel::find()->localized(Yii::$app->urlManager->getCurLangCode())->where(['or', 'news_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])->status(NewsModel::STATUS_ON)->one();
        if($news) {
