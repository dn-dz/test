...
class Gallery extends \yii\easyii\components\API
{
    ...
    public function api_last($limit = 1, $where = null)
    {
        ...
        $query = Photo::find()->localized(Yii::$app->urlManager->getCurLangCode())->where(['class' => Category::className()])->sort()->limit($limit);
        if($where){
    ...
    {
        $category = Category::find()->localized(Yii::$app->urlManager->getCurLangCode())->where(['or', 'category_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])->status(Category::STATUS_ON)->one();
    ...
    {
        return Photo::find()->where(['photo_id' => $id])->localized(Yii::$app->urlManager->getCurLangCode())->one();
    }
}